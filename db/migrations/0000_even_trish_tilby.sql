...
CREATE TABLE "knowledge_base_shares" (
	...
	"created_at" timestamp with time zone DEFAULT timezone('utc'::text, now()),
	CONSTRAINT "unique_kb_email_share" UNIQUE("knowledge_base_id","shared_with_email")
);
--> statement-breakpoint
ALTER TABLE "knowledge_base_shares" ENABLE ROW LEVEL SECURITY;--> statement-breakpoint
...
ALTER TABLE "knowledge_base_shares" ADD CONSTRAINT "knowledge_base_shares_knowledge_base_id_knowledge_bases_id_fk" FOREIGN KEY ("knowledge_base_id") REFERENCES "public"."knowledge_bases"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "knowledge_base_shares" ADD CONSTRAINT "knowledge_base_shares_shared_by_user_id_users_id_fk" FOREIGN KEY ("shared_by_user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE POLICY "Users can view shares for their knowledge bases" ON "knowledge_base_shares" AS PERMISSIVE FOR SELECT TO public USING ((
        EXISTS (
...
CREATE POLICY "Users can create shares for their knowledge bases" ON "knowledge_base_shares" AS PERMISSIVE FOR INSERT TO public WITH CHECK ((
        EXISTS (
          SELECT 1 FROM knowledge_bases kb 
          WHERE kb.id = knowledge_base_id 
          AND kb.user_id = auth.uid()
...
CREATE POLICY "Users can delete shares for their knowledge bases" ON "knowledge_base_shares" AS PERMISSIVE FOR DELETE TO public USING ((
        EXISTS (
          SELECT 1 FROM knowledge_bases kb 
          WHERE kb.id = knowledge_base_id 
          AND kb.user_id = auth.uid()
...
CREATE POLICY "Users can delete their own knowledge bases" ON "knowledge_bases" AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = user_id));--> statement-breakpoint
CREATE POLICY "Users can view their own config" ON "user_config" AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Users can insert their own config" ON "user_config" AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Users can update their own config" ON "user_config" AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Users can delete their own config" ON "user_config" AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Users can view their own profile" ON "users" AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Anyone can view public profile info" ON "users" AS PERMISSIVE FOR SELECT TO public USING (true);--> statement-breakpoint
...
CREATE POLICY "Users can update their own profile" ON "users" AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() = id));--> statement-breakpoint
CREATE POLICY "Users can delete their own profile" ON "users" AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = id));